                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                                    <h4 class="mb-sm-0 font-size-18">Bills receipted</h4>

                                    <div class="page-title-right">
                                        <ol class="breadcrumb m-0">
                                            <li class="breadcrumb-item"><a @click="gotTo('dashboard')">Dashboards</a>
                                            </li>
                                            <li class="breadcrumb-item active">Bill receipted</li>
                                        </ol>
                                    </div>
    ...
    export default {
        ...
        mounted() {
            ...
            this.$watch('filter', function (newValue) {
                ...
                if (newValue === 'filter-department') {
                    this.getDepartment()
                }
                if (newValue === 'filter-stream') {
                    this.getStreams()
                }
        ...
        return pages;
           ...
           // define a method to go to a specific page
                ...
                execute(data, biller)
                    .then((res) => {
                        if (res.data.success) {
                            ...
                            for (let i = 0; i < res.data.data.subCounties.length; i++) {
                                let subCounty = res.data.data.subCounties[i];
                                let item = {
                ...
                execute(data, biller)
                    .then((res) => {
                        if (res.data.success) {
                            ...
                            for (let i = 0; i < res.data.data.wards.length; i++) {
                                let ward = res.data.data.wards[i];
                                let item = {
                                    id: ward.wardID,
                                    value: ward.wardName
                                };
                    ...
                    .then((res) => {
                        if (res.data.success) {
                            for (let i = 0; i < res.data.data.departments.length; i++) {
                                let user = res.data.data.departments[i];
                                let item = {
            ...
            getStreams() {
                this.itemList.splice(0)
                const data = new FormData();
                data.append("function", "getStreams")
                execute(data, biller)
                    .then((res) => {
                        if (res.data.success) {
                            for (let i = 0; i < res.data.data.streams.length; i++) {
                                let user = res.data.data.streams[i];
                                let item = {
                                    id: user.incomeTypeDescription,
                                    value: user.incomeTypeDescription
                                };
