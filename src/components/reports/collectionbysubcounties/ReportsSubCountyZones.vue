                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                                <h4 class="mb-sm-0 font-size-18">Revenue Collections For Homa Bay Town Sub county for the period 1 Jan 2023 to 31 Jan 2023</h4>
                                ...
                                <div class="page-title-right">
                                    <ol class="breadcrumb m-0">
                                        ...
                                        <li class="breadcrumb-item"><a @click="gotTo('reports-sub-county')" href="javascript: void(0)">Revenue collection by sub county</a></li>
                                    ...
                                    <div class="btn-toolbar p-3 d-flex justify-content-between align-items-center w-100" role="toolbar">
                                      <div class="flex-grow-1">
                                        ...
                                        <div class="d-flex">
                                            ...
                                            <button @click="downloadReceiptedByZonesPaginated()" type="button" class="btn btn-primary dropdown-toggle option-selector me-3 text-capitalize">
                                ...
                                <div class="card-body">
                                    <div class="table-responsive overflow-visible">
                                        ...
                                        <table class="table align-middle table-hover  contacts-table table-striped " id="">
                                            ...
                                            <tbody>
                                                <tr v-for="(item, index) in revenueZones" :value="item" :key="index">
                                                    <td class="text-capitalize text-nowrap"><strong>{{index+1}}.</strong></td>
                                                    <td @click="gotToDetail('reports-sub-county-zones-details',item.zone)" class="text-capitalize text-nowrap">
                                                        <a href="javascript:void(0)">{{item.zone}}</a>
                                                    </td>
                                                    <td>{{item.wardName}}</td>
                                                    <td>{{item.subCountyName}}</td>
                                                    <td class="text-capitalize text-nowrap">{{item.count}}</td>
                                                    <td class="text-capitalize text-nowrap">{{item.collectors}}</td>
                                                    <td class="text-capitalize text-nowrap">{{item.inspectors}}</td>
                                                    <td class="text-capitalize text-nowrap">{{item.enforcers}}</td>
                                                    <td class="text-capitalize text-nowrap text-right">KES {{numberFormat(item.amount)}}</td>
                                                    <td>
                                                        <div @click="gotToDetail('reports-sub-county-zones-details',item.zone)" class="d-flex justify-content-end">
                                                            <a type="button" href="javascript:void(0)" class="btn btn-primary btn-sm waves-effect waves-light text-nowrap me-3">View Details</a>
                                                            <div class="dropdown">
                                        ...
                                        <div v-if="revenueZones.length<1 & loading===false" class="card-body d-sm-none d-md-block px-5 d-flex flex-column justify-items-center align-items-center text-center">
                                            <div class="p-5 py-0 pt-3">
    ...
    export default {
        ...
        watch: {
            ...
            pageSize(newPageSize) {
                this.getReceiptedByZonesPaginated()//Todo to change here
            ...
            search(){
                this.getReceiptedByZonesPaginated()
            },
            documentType(){
...
// return the array of pages
            ...
            prevPage() {
                ...
                if (this.currentPage > 1) {
                    ...
                    this.currentPage--;
                    this.getReceiptedByZonesPaginated()//Todo change
                }
            },
            ...
            getReceiptedByZonesPaginated() {
                this.revenueZones.splice(0)
                this.loading = true
                ...
                execute(data, biller)
                    .then((res) => {
                        ...
                        if (res.data.success) {
                            this.revenueZones = res.data.data.revenueZones
                            //todo pagination
                          ...
                          /*  //Initialize the accumulator to 0
                            this.totalAmount = this.revenueZones.reduce((acc, cur) => {
                                let amount = Number(cur.amount);
                                return acc + amount;
                            }, 0);
                            this.totalAgents = this.revenueZones.reduce((acc, cur) => {
                                let agents = Number(cur.agents);
                                return acc + agents;
                            }, 0);
                            this.totalTransactions = this.revenueZones.reduce((acc, cur) => {
            ...
            downloadReceiptedByZonesPaginated(){
                const data = new FormData();
                data.append("function", "downloadReceiptedByZonesPaginated");
                data.append("page", this.currentPage);
                data.append("rows_per_page", this.pageSize);
                data.append("status", "PAID");
                data.append("dateFrom", this.dateFrom);
